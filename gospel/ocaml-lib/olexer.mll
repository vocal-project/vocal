...
let add_docstring_comment ds =
  let com =
    ("*" ^ Odocstrings.docstring_body ds, Odocstrings.docstring_loc ds)
  in
    add_comment com
  ...
  | "(**"
      { let s, loc = with_comment_buffer comment lexbuf in
        if !handle_docstrings then
          DOCSTRING (Odocstrings.docstring s loc)
        else
          COMMENT ("*" ^ s, loc)
      }
  | "(**" (('*'+) as stars)
  ...
  | "(*" (('*'*) as stars) "*)"
      { if !handle_docstrings && stars="" then
         (* (**) is an empty docstring *)
          DOCSTRING(Odocstrings.docstring "" (Location.curr lexbuf))
        else
...

{
  ...
  and docstring = Odocstrings.docstring

  let token lexbuf =
    let post_pos = lexeme_end_p lexbuf in
    let attach lines docs pre_pos =
      let open Odocstrings in
    ...
    let rec loop lines docs lexbuf =
      ...
      | EOL ->
          ...
          loop lines' docs lexbuf
      | DOCSTRING doc ->
          Odocstrings.register doc;
          add_docstring_comment doc;
          let docs' =
            if Odocstrings.docstring_body doc = "/*" then
              match docs with
              | Initial -> Before([], [doc], [])
