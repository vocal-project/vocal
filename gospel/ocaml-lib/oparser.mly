...
/* Based on the OCml parser, commit 8c75b5f1d6133585bc */
...
type let_bindings =
  { lbs_bindings: let_binding list;
    lbs_rec: rec_flag;
    lbs_extension: string Oasttypes.loc option;
    lbs_loc: Location.t }
...
(* Alternatively, we could keep the generic module type in the Oparsetree
...
%token <Odocstrings.docstring> DOCSTRING
%token <string * Location.t> GOSPELSPEC
...
%nonassoc gospel_post_priority
%nonassoc LBRACKETAT GOSPELSPEC
...
%type <Oparsetree.structure> implementation
%start interface                        /* for interface files */
%type <Oparsetree.signature> interface
%start toplevel_phrase                  /* for interactive use */
%type <Oparsetree.toplevel_phrase> toplevel_phrase
%start use_file                         /* for the #use directive */
%type <Oparsetree.toplevel_phrase list> use_file
%start parse_core_type
%type <Oparsetree.core_type> parse_core_type
%start parse_expression
%type <Oparsetree.expression> parse_expression
%start parse_pattern
%type <Oparsetree.pattern> parse_pattern
%%
...
post_item_attribute:
  LBRACKETATAT attr_id payload RBRACKET
    { Attr.mk ~loc:(make_loc $sloc) $2 $3 }
  | LBRACKETATAT GOSPEL STRING RBRACKET
      { let loc = make_loc $loc($3) in
        let loc = {loc with loc_start={loc.loc_start
          with pos_cnum=loc.loc_start.pos_cnum+1}} in
        Attr.mk ~loc {txt=gospel;loc=make_loc $loc($2)}  (PGospel (fst $3)) }
  | GOSPELSPEC
    { let s,loc = $1 in
      ...
      Attr.mk ~loc {txt=gospel;loc}  (PGospel s) }
;
floating_attribute:
  LBRACKETATATAT attr_id payload RBRACKET
    { mark_symbol_docs $sloc;
      Attr.mk ~loc:(make_loc $sloc) $2 $3 }
  | LBRACKETATATAT GOSPEL STRING RBRACKET
      { let loc = make_loc $loc($3) in
        let loc = {loc with loc_start={loc.loc_start
          with pos_cnum=loc.loc_start.pos_cnum+1}} in
        Attr.mk ~loc {txt=gospel;loc=make_loc $loc($2)}  (PGospel (fst $3)) }
...
;
post_item_attributes:
    /* empty */ %prec gospel_post_priority { [] }
