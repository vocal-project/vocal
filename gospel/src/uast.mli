...
and s_expression_desc =
  ...
  | Sexp_open of Parsetree.open_declaration * s_expression
        (* M.(E)
           let open M in E
           let! open M in E *)
  | Sexp_letop of Parsetree.letop
...
and s_module_expr_desc =
  ...
  | Smod_structure of s_structure
        (* struct ... end *)
  | Smod_functor of s_functor_param * s_module_expr
        (* functor(X : MT1) -> ME *)
...
and s_functor_param =
  | Sunit
        (* () *)
  | Snamed of string option loc * s_module_type
        (* (X : MT)          Some X, MT
           (_ : MT)          None, MT *)
