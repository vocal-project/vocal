...
let type_spec ?(extra_spec=[]) t =
  ...
  List.iter (fun (c,_)     -> unsupported_gospel.typ unsupported_gospel c) t.ptype_params;
  List.iter (fun (c1,c2,_) -> unsupported_gospel.typ unsupported_gospel c1;
                              unsupported_gospel.typ unsupported_gospel c2) t.ptype_cstrs;
  unsupported_gospel.type_kind unsupported_gospel t.ptype_kind;
  (match t.ptype_manifest with
     None -> ()
   | Some m -> unsupported_gospel.typ unsupported_gospel m);
...
let val_description v =
  (* no specification attached to unsupported fields *)
  unsupported_gospel.typ unsupported_gospel v.pval_type;
...
let with_constraint c =
  unsupported_gospel.with_constraint unsupported_gospel c;
  ...
  | {psig_desc;psig_loc=sloc} :: xs ->
     ...
     let current_specs = match psig_desc with
       ...
       | Psig_open o ->
          ...
          unsupported_gospel.open_description unsupported_gospel o;
          {sdesc=Sig_open o;sloc} :: specs
       | Psig_include i ->
          let attrs, _ = get_spec_attrs i.pincl_attributes in
          let i = {i with pincl_attributes = attrs} in
          unsupported_gospel.include_description unsupported_gospel i;
       ...
       | Psig_class c ->
          let c,specs =
            List.fold_right (fun cd (cl,specl) ->
                ...
                unsupported_gospel.class_description unsupported_gospel c;
                c::cl, specs @ specl
              ) c ([],[]) in
       ...
       | Psig_class_type c ->
          let c, _ =
            List.fold_right (fun cd (cl,specl) ->
                let attrs,specs = get_spec_attrs cd.pci_attributes in
                let c = {cd with pci_attributes = attrs} in
                unsupported_gospel.class_type_declaration unsupported_gospel c;
                c::cl, specs @ specl
              ) c ([],[]) in
...
and module_type m =
  unsupported_gospel.attributes unsupported_gospel m.pmty_attributes;
...
and s_module_expr {pmod_desc; pmod_loc; pmod_attributes} =
  let spmod_desc = match pmod_desc with
    ...
    | Pmod_functor (func_param, mod_expr) ->
        Smod_functor (s_functor_param func_param, s_module_expr mod_expr)
...
and s_functor_param = function
  | Unit -> Sunit
  | Named (id, mod_type) -> Snamed (id, module_type mod_type)
...
and s_value_binding vb_list =
  ...
  let val_spec v =
    let spec, _ = split_attr v.pvb_attributes in
    let spec = List.map attr2spec spec in
